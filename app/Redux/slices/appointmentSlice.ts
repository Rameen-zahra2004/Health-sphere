...
export const fetchPatientAppointments = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("appointment/fetchPatient", async (_, { rejectWithValue }) => {
  try {
    return await getPatientAppointments();
  } catch (err) {
    return rejectWithValue(getErrorMessage(err));
...
export const fetchDoctorAppointments = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("appointment/fetchDoctor", async (_, { rejectWithValue }) => {
  try {
    return await getDoctorAppointments();
  } catch (err) {
    return rejectWithValue(getErrorMessage(err));
...
export const updateAppointmentStatusThunk = createAsyncThunk<
  ...
  { id: string; status: Appointment["status"] },
  { rejectValue: string }
>("appointment/updateStatus", async ({ id, status }, { rejectWithValue }) => {
  try {
    return await updateAppointmentStatus(id, status);
  } catch (err) {
    return rejectWithValue(getErrorMessage(err));
...
const appointmentSlice = createSlice({
  ...
  reducers: {
    ...
    socketAppointmentCreated: (state, action: PayloadAction<Appointment>) => {
    ...
    socketAppointmentUpdated: (state, action: PayloadAction<Appointment>) => {
      const index = state.appointments.findIndex(
        (a) => a._id === action.payload._id
      );
    ...
    socketAppointmentCancelled: (state, action: PayloadAction<string>) => {
      state.appointments = state.appointments.filter(
        (a) => a._id !== action.payload
      );
    },

    socketAppointmentCompleted: (state, action: PayloadAction<Appointment>) => {
      const index = state.appointments.findIndex(
        (a) => a._id === action.payload._id
      );
